/* Power saving code, wakes on RTC-based interrupt
 * 12-31-20 CDW
 * code based on tutorials at https://thekurks.net/blog/2018/1/24/guide-to-arduino-sleep-mode
 * RTC setup based on example from Adafruit RTClib: pcf8523Countdown
 * RTC is provided by Adafruit datalogger shield with PCF8523
 * INT/SQW pin from RTC shield must be connected to arduino pin 2 using jumper wire to trigger the interrupt
 */
...
//Pin definitions
//Note that on Aruino Uno, pins 2 and 3 are HW interrupt capable
...
// Connect to RTC
  ...
  rtc.enableCountdownTimer(PCF8523_FrequencySecond, 600);     // 10 minute
...
// Added these for sensors and SD card (SD card on pin 10). 
  pinMode(2, INPUT);
  digitalWrite(2,HIGH);
...
// inserted SD card write here 
  pinMode(10, OUTPUT);  // default chip select pin is set to output, even if you don't use it:
...
// Get data from DHT-11 using dhtRead()
 int dhtData = dhtRead();    // Read DHT sensor
 int val = readSensor(); // Take leaf wetness reading 
 
// Determine status of leaf wetness sensor FC-37 
  if (val) {
    Serial.print("Dry");
  ...
  //display current count of pulse counter
...
logfile.close();
   ...
   g_count = 0; //reset count for next period
...
// End of logging time and sensor data. 
 ...
 delay(2000); //wait 0.1 second before going to sleep
